...
export class Router {
    ...
    getCurrentPath (): string {
        ...
        let originalRoute: string | null = window.sessionStorage.getItem('html4.route');
        if (originalRoute) {
    ...
    start (): boolean {
        ...
        if (window.history && 'pushState' in window.history) {
            ...
            window.addEventListener('popstate', (ev: PopStateEvent): void => {
                ...
                let appRoute: string = window.location.pathname.substr(router.appBase.length);
                for (let i: number = 0; i < router.routes.length; i++) {
            ...
            $('a').on('click',function (event: JQuery.Event): void {
                console.log('a click...');
                let url: string | undefined = $(this).attr('href');
                console.log('url :', url);
    ...
    navigateTo (fullPath: string): boolean {
        ...
        if (fullPath.substr(0, this.appBase.length) === this.appBase) {
            let appRoute: string = fullPath.substr(this.appBase.length);
